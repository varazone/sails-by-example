...
const asTuple = <T extends z.ZodTypeAny>(schema: T[]) =>
  z.tuple(schema as [T, ...T[]]);
...
const getPayloadSchema = (
  ...
  const getSchema = (def: ISailsTypeDef): z.ZodType<unknown> => {
    if (def.isPrimitive) {
      return def.asPrimitive.isBool ? z.boolean() : z.string().trim();
    }

    if (def.isOptional) {
      return z.union([z.null(), getSchema(def.asOptional.def)]);
    }
    ...
    if (def.isFixedSizeArray) {
      return z.array(getSchema(def.asFixedSizeArray.def)).length(
        def.asFixedSizeArray.len,
      );
    }
    ...
    if (def.isUserDefined) {
      return getSchema(sails.getTypeDef(def.asUserDefined.name));
    }
    ...
    if (def.isStruct) {
      ...
      const fieldsSchema = fields.map((field, index) =>
        [field.name || index, getSchema(field.def)] as const
      );
    ...
    if (def.isEnum) {
      const variants = def.asEnum.variants.map((variant) =>
        z.object(
          {
            [variant.name]: variant.def ? getSchema(variant.def) : z.null(),
          } as const,
        )
      );
  ...
  const result = args.map(({ typeDef }, index) =>
    [index, getSchema(typeDef)] as const
  );

  return z.object(Object.fromEntries(result)).transform((value) =>
    encode(...Object.values(value))
  );
};

...
const getType = (def: ISailsTypeDef): string => {
  ...
  if (def.isResult) {
    return `Result<${getType(def.asResult.ok.def)}, ${
      getType(def.asResult.err.def)
    }>`;
  }
  if (def.isVec) return `Vec<${getType(def.asVec.def)}>`;
  if (def.isFixedSizeArray) {
    return `[${
      getType(def.asFixedSizeArray.def)
    }; ${def.asFixedSizeArray.len}]`;
  }
  if (def.isMap) {
    return `BTreeMap<${getType(def.asMap.key.def)}, ${
      getType(def.asMap.value.def)
    }>`;
  }
  ...
  if (def.isStruct) {
    if (def.asStruct.isTuple) {
      return `(${
        def.asStruct.fields.map((field) => getType(field.def)).join(", ")
      })`;
    }

    const result = def.asStruct.fields.map((
      field,
    ) => [field.name, getType(field.def)]);

    return JSON.stringify(Object.fromEntries(result));
  }

  if (def.isEnum) {
    if (!def.asEnum.isNesting) {
      return JSON.stringify({ _enum: def.asEnum.variants.map((v) => v.name) });
    }

    const result = def.asEnum.variants.map((
      variant,
    ) => [variant.name, variant.def ? getType(variant.def) : "Null"]);

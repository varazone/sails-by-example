...
const SearchForm = <T extends z.ZodTypeAny>(
  { placeholder, className, disabled, onSubmit, getSchema }: Props<T>,
) => {
  const schema = z.object({
    [FIELD_NAME.QUERY]: asOptionalField(
      getSchema ? getSchema(QUERY_SCHEMA) : QUERY_SCHEMA,
    ),
  });
  ...
  const handleSubmit = form.handleSubmit((values) =>
    onSubmit(values[FIELD_NAME.QUERY])
  );

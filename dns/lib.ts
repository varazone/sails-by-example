...
export class Dns {
  constructor(private _program: DnsProgram) {}
  ...
  public async getContractInfoByName(
    ...
    const payload = this._program.registry.createType(
      "(String, String, String)",
      ["Dns", "GetContractInfoByName", name],
    ).toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
  ...
  public async getNameByProgramId(
    ...
    const payload = this._program.registry.createType(
      "(String, String, [u8;32])",
      ["Dns", "GetNameByProgramId", program_id],
    ).toHex();
    const reply = await this._program.api.message.calculateReply({
      destination: this._program.programId,
      origin: originAddress ? decodeAddress(originAddress) : ZERO_ADDRESS,
  ...
  public subscribeToNewProgramAddedEvent(
    callback: (
      data: { name: string; contract_info: ContractInfo },
    ) => void | Promise<void>,
  ): Promise<() => void> {
        ...
        if (
          ...
          callback(
            this._program.registry.createType(
              ...
              message.payload,
            )[2].toJSON() as unknown as { name: string },
          );
        }
      },
    );
